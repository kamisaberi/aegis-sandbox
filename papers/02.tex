...
Unlike direct prompt injection (where a user directly inputs adversarial prompts to a chatbot), indirect prompt injection occurs when the LLM processes data from a third-party source. For example, an agent summarizing a web page might encounter:
...
To neutralize IMDS exfiltration without degrading execution speed, our framework hooks into the Linux kernel using \textbf{eBPF (Extended Berkeley Packet Filter)} at the \texttt{cgroup/connect4} socket layer.
...
\begin{lstlisting}[caption={YAML Policy Excerpt for Anti-SSRF Protection}]
network_rules:
...
The eBPF kernel socket filter introduces a negligible \textbf{+0.16\% connection latency overhead} ($2$ microseconds per \texttt{connect()} call), representing a \textbf{1,800x speed improvement over traditional user-space security proxies}.
